...
namespace narray_ops {
    ...
    template <class T,class S>
    void getmax(colib::narray<T> &out,S in) {
        for(int i=0;i<out.length1d();i++)
            out.at1d(i) = max_(out.at1d(i),in);
    }

    template <class T,class S>
    void getmin(colib::narray<T> &out,S in) {
        for(int i=0;i<out.length1d();i++)
            out.at1d(i) = min_(out.at1d(i),in);
    }

    template <class T,class S>
    void getmax(colib::narray<T> &out,colib::narray<S> &in) {
    ...
    template <class T,class S>
    void getmin(colib::narray<T> &out,colib::narray<S> &in) {
        CHECK_ARG(samedims(out,in));
        for(int i=0;i<out.length1d();i++)
