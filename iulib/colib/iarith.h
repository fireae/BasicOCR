...
#define returnoops(S,V) throw S
    ...
    inline double getmin(double x,double y) { return x<y?x:y; }
...
// C99 primitives require gcc-3.0 or above
    ...
    inline double exp_lo(double x) {
        ...
        return getmax(0.0,prevfp(exp(x)));
    }
    ...
    struct interval {
        ...
        void intersect(interval other) {
            lo = getmax(lo,other.lo);
            hi = getmin(hi,other.hi);
        }
        bool empty() {
            return lo>hi;
        }
    ...
    inline interval sqinfluence(interval a,double t) {
        ...
        return interval(getmax(0.0,1-a.hi*a.hi/t/t),getmax(0.0,1-a.lo*a.lo/t/t));
    }

    inline bool nonempty(interval x) { return x.hi>=x.lo; }
    inline interval intersect(interval a,interval b) {
        return interval(getmax(a.lo,b.lo),getmin(a.hi,b.hi));
    }
    inline interval iunion(interval a,interval b) {
        return interval(getmin(a.lo,b.lo),getmax(a.hi,b.hi));
    }

    inline interval iintersect(interval a,interval b) {
        interval result = interval(getmax(a.lo,b.lo),getmin(a.hi,b.hi));
    ...
    inline interval abs(interval x) {
        ...
        return interval(0.0,getmax(dabs(x.lo),dabs(x.hi)));
    }
    ...
    inline interval interval::operator*(interval b) const {
        ...
        if(a>=0.0) {
            ...
            return interval(getmin(mul_lo(a.hi,b.lo),mul_lo(a.lo,b.hi)),
                            getmax(mul_hi(a.lo,b.lo),mul_hi(a.hi,b.hi)));
        }
    }
    ...
    inline interval sin(interval x) {
        ...
        else result.hi = getmax(sin_hi(x.lo),sin_hi(x.hi));
        if(x.contains(3*M_PI/2)||x.contains(7*M_PI/2)) result.lo = -1.0;
        else result.lo = getmin(sin_lo(x.lo),sin_lo(x.hi));
        return result;
    }
    inline interval cos(interval x) {
        ...
        else result.hi = getmax(cos_hi(x.lo),cos_hi(x.hi));
        if(x.contains(M_PI)||x.contains(3*M_PI)) result.lo = -1.0;
        else result.lo = getmin(cos_lo(x.lo),cos_lo(x.hi));
        return result;
    }
    ...
    inline interval getmin(double a,interval b) {
        return interval(getmin(a,b.lo),getmin(a,b.hi));
    }
    inline interval getmax(double a,interval b) {
        return interval(getmax(a,b.lo),getmax(a,b.hi));
    }
    inline interval getmin(interval a,double b) {
        return interval(getmin(a.lo,b),getmin(a.hi,b));
    }
    inline interval getmax(interval a,double b) {
        return interval(getmax(a.lo,b),getmax(a.hi,b));
    }
    inline interval getmin(interval a,interval b) {
        return interval(getmin(a.lo,b.lo),getmin(a.hi,b.hi));
    }
//inline interval getmax(interval a,interval b) {
    inline interval getmax(interval a,interval b) {
        return interval(getmax(a.lo,b.lo),getmax(a.hi,b.hi));
    }
    ...
    inline interval angle_difference(interval a,double b) {
        ...
        while(b<0) b += 2*M_PI;
        interval range = getmin(abs(a-b),getmin(abs(a-(b-2*M_PI)),abs(a-(b+2*M_PI))));
        return range;
    }
    ...
    inline interval orientation_difference(interval a,double b) {
        ...
        while(b>=M_PI) b -= M_PI;
        while(b<0) b += M_PI;
        interval range = getmin(abs(a-b),getmin(abs(a-(b-M_PI)),abs(a-(b+M_PI))));
        return range;
    }
    ...
    struct vec2 {
        ...
        interval distance(const vec2 &other) {
            return sqrt(getmax(0.0,distance_squared(other)));
        }
    };
#endif
    ...
    struct vec2i {
        ...
        float minwidth() {
            return getmin(x.width(),y.width());
        }
        float maxwidth() {
            return getmax(x.width(),y.width());
        }
    ...
    inline interval norm(vec2i v) {
        return sqrt(getmax(0.0,sqr(v.x)+sqr(v.y)));
    }

    inline double norm(vec2 v) {
        return ::sqrt(getmax(0.0,sqr(v.data[0])+sqr(v.data[1])));
    }
    ...
    struct vecni {
        ...
        vecni<N> splitgetmax(int which) const {
