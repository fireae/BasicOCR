...
namespace colib {
    ...
    template <class T>
    int rowarggetmax(narray<T> &values,int i) {
    ...
    template <class T>
    int rowarggetmin(narray<T> &values,int i) {
        if(values.dim(1)<1) return -1;
        int mj = 0;
        T mv = values(i,0);
        for(int j=1;j<values.dim(1);j++) {
    ...
    T rowdist_euclidean(narray<T> &values,int i,narray<T> &v) {
        return (T)sqrt2(rowdist_euclidean_squared(values,i,v));
    }
    ...
    template <class T>
    T &rowgetmax(narray<T> &values,int i) {
        return values(i,rowarggetmax(values,i));
    }

    template <class T>
    T &rowgetmin(narray<T> &values,int i) {
        return values(i,rowarggetmin(values,i));
    }
