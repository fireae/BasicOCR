...
namespace colib {
    ...
    struct rectangle {
        ...
        void intersect(rectangle other) {
            if(empty()) return;
            x0 = getmax(x0,other.x0);
            y0 = getmax(y0,other.y0);
            x1 = getmin(x1,other.x1);
            y1 = getmin(y1,other.y1);
        }
        void include(rectangle other) {
            if(empty()) {
                *this = other;
            } else {
                x0 = getmin(x0,other.x0);
                y0 = getmin(y0,other.y0);
                x1 = getmax(x1,other.x1);
                y1 = getmax(y1,other.y1);
            }
        }
