...
//win32        if(0) data[0]+0;
        ...
        if(data.length1d()>0) {
			debugf("narray_read", "data.length1d() = %d and sizeof data.at1d(0) = %d\n", data.length1d(), sizeof data.at1d(0));
#ifndef _WIN32
            CHECK((int)fread(&data.at1d(0),sizeof data.at1d(0),
                             data.length1d(),stream)==data.length1d());
#else
			int result = (int)fread(&data.at1d(0),sizeof data.at1d(0),
                             data.length1d(),stream);
			if (result != data.length1d()) debugf("narray_read","Didn't read enough, only read %d\n", result);
			if (feof(stream)) debugf("narray_read", "hit EOF\n");
			if (ferror(stream)) perror("[narray_read] error:");
			CHECK(result==data.length1d());
#endif
        }
    }
