...
namespace colib {
    ...
    template <class T>
    inline int arggetmax(narray<T> &a) {
    ...
    template <class T>
    inline int arggetmin(narray<T> &a) {
        CHECK_ARG(a.rank()==1 && a.dim(0)>0);
        T value = a(0);
        int index = 0;
        for(int i=1;i<a.dim(0);i++) {
