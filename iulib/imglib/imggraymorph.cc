...
namespace iulib {
    ...
    int maxdifference(bytearray &image, bytearray &image2, int cx, int cy) {
        ...
        for (int i=cx; i<w-cx; i++)
            for (int j=cy; j<h-cy; j++) {
                d = colib::getmax(d, image(i, j)-image2(i, j));
            }
        return d;
    }
    ...
    void minshift(bytearray &image, bytearray &image2, int dx, int dy,
        ...
        for (int i=0; i<w; i++)
            for (int j=0; j<h; j++) {
                image.at(i, j) = colib::getmin(image(i, j), bc(ext(image2, i-dx, j-dy)
                        +(255-offset)));
            }
    }
    ...
    void maxshift(bytearray &image, bytearray &image2, int dx, int dy,
        ...
        for (int i=0; i<w; i++)
            for (int j=0; j<h; j++) {
                image.at(i, j) = colib::getmax(image.at(i, j), bc(ext(image2, i-dx, j-dy)
                        -(255-offset)));
            }
    }
