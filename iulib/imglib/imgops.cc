...
namespace iulib {
    ...
    void tighten(narray<T> &image) {
        ...
        for (int x=0; x<image.dim(0); x++) {
            for (int y=0; y<image.dim(1); y++) {
                ...
                x1 = colib::getmax(x1,x);
                y1 = colib::getmax(y1,y);
            }
        }
        if (x0>x1||y0>y1)
            return;
    ...
    void extract_subimage(narray<T> &out,narray<S> &image,int x0,int y0,int x1,int y1) {
        CHECK(x0<=x1);
        CHECK(y0<=y1);
        x0 = colib::getmax(x0,0);
        y0 = colib::getmax(y0,0);
        x1 = colib::getmin(x1,image.dim(0));
        y1 = colib::getmin(y1,image.dim(1));
        int w = x1-x0;
        int h = y1-y0;
        out.resize(w,h);
        for (int i=0; i<w; i++)
    ...
    void expand_range(narray<T> &image,float lo,float hi) {
        float mn = colib::getmin(image);
        float mx = colib::getmax(image);
        if (mn==mx) {
            fill(image,T(lo));
            return;
        }
