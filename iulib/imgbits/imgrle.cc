        ...
        void rle_shift_d1(RLEImage &image,int d) {
            ...
            for(int i=0;i<w;i++) {
                ...
                for(int j=0;j<line.length();j++) {
                    ...
                    RLERun r = line(j);
                    r += d;
                    r.start = colib::getmax(0,r.start);
                    r.end = colib::getmin(h,r.end);
...
namespace imgrle {
    ...
    void rle_transpose_runs(RLEImage &out,RLEImage &in) {
        ...
        for(int here=0;here<=in.dim(0);here++) {
            ...
            int jl = 0;
            for(;;) {
#define finish(FROM,TO) do{int TO_=colib::getmin(TO,out.nlines()); for(int K=FROM;K<TO_;K++) out.line(K).push(RLERun(last,here));}while(0)
