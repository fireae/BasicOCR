...
#define DEFAULT_DATA_DIR "/usr/local/share/ocropus/models/"
    ...
    int main_lines2fsts(int argc,char **argv) {
        ...
        Glob files(pattern);
        int finished = 0;
        int nfiles = getmin(files.length(),nrecognize);
...
#pragma omp critical (finished_counter)
    ...
    int main_recognize1(int argc,char **argv) {
        ...
        for(int i=3;i<argc;i++) {
            ...
            //char* ss = new char[str.size()];					///Gary TODO memory leak
			//narray<char> s;
            //str.utf8Encode(ss, str.size());
			//for(int i = 0; i < s.length ; i++)
			//s += ss[0];
            //s.push(0);
            //printf("%s\t%s\n",argv[i],&s[0]);
        ...
        for(int index=0;index<files.length();index++) {
            try {
                ...
                nustring nutranscript;
                {
                    ...
                    if(nutranscript.length()!=getmax(cseg))
                        throwf("transcript doesn't agree with cseg (transcript %d, cseg %d)",
                            nutranscript.length(),getmax(cseg));
                }
                ...
                {
                    char *transcript = new char[nutranscript.size()];
					nutranscript.utf8Encode(transcript, nutranscript.size());			///Gary TODO Posibily broken
                    debugf("transcript","%s (%d) [%2d,%2d] %s\n",filename,total_chars,
                           nutranscript.length(),getmax(cseg),transcript);
                    free(transcript);
                }
                if(total_chars>=next) {
