...
namespace ocropus {
    ...
    void plot_hist(FILE *stream, floatarray &hist){
        ...
        fill(image,0xff);
        for(int x=0; x<len; x++){
            int top = colib::getmin(maxval-1,int(hist[x]));
            for(int y=0; y<top; y++)
                image(x,y) = 0;
        }
    ...
    void align_segmentation(intarray &segmentation,narray<rectangle> &bboxes) {
        ...
        int nsegs = colib::getmax(segmentation)+1;
        intarray counts;
...
//         }
    ...
    void ocr_bboxes_to_charseg(intarray &cseg,narray<rectangle> &bboxes,intarray &segmentation) {
        make_line_segmentation_black(segmentation);
        CHECK_ARG(colib::getmax(segmentation)<100000);
        intarray counts(colib::getmax(segmentation)+1,bboxes.length());
        fill(counts,0);
        ...
        intarray valuemap(colib::getmax(segmentation)+1);
        fill(valuemap,0);
        for(int i=1;i<counts.dim(0);i++)
            valuemap(i) = rowarggetmax(counts,i)+1;
    ...
    int binarize_simple(bytearray &result, bytearray &image) {
        int threshold = (colib::getmax(image)+colib::getmin(image))/2;
    ...
    void binarize_with_threshold(bytearray &in, int threshold) {
        binarize_with_threshold((bytearray)NULL, in, threshold);		//Gary TODO break?
		return;
    }
