    ...
    void elastic_transform(floatarray &out, floatarray &in,
        ...
        for(int x = 0; x < in.dim(0); x++) {
            for(int y = 0; y < in.dim(1); y++) {
                float nx = x + dx(x,y);
                float ny = y + dy(x,y);
                int x0 = getmin(getmax(int(floor(nx)), 0), in.dim(0) - 1);
                int y0 = getmin(getmax(int(floor(ny)), 0), in.dim(1) - 1);
                int x1 = getmin(x0 + 1, in.dim(0) - 1);
                int y1 = getmin(y0 + 1, in.dim(1) - 1);
                float xx = nx - x0;
                float yy = ny - y0;
