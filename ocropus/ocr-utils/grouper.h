...
namespace ocropus {
    ...
    struct IGrouper : IComponent {
        ...
        virtual void extractSliced(bytearray &out,bytearray &source,
                                   colib::byte dflt,int index,int grow=0) = 0;
        ...
        virtual void setClass(int i,ustrg &cls,float cost);
        ...
        virtual void setSpaceCost(int index,float yes,float no);

        // Extract the lattice corresponding to the classifications
        // stored in the Grouper.

        virtual void getLattice(IGenericFst &fst);
        ...
        virtual int getGtClass(int index);
        virtual int getGtIndex(int index);
