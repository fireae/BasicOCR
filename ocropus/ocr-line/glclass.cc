...
namespace glinerec {
    ...
    double nearest_neighbor_error(IDataset &data,int ntrials=1000) {
        int total = 0;
		srand(42);
        ntrials = getmin(data.nsamples(),ntrials);
        for(int i=0;i<data.nsamples();i++) {
            // FIXME use sampling without replacement
            int ii = rand()%data.nsamples();
            floatarray u;
            data.input(u,ii);
            int n = data.nsamples();
            floatarray dists(n);
#pragma omp parallel for shared(dists)
            ...
            int index = arggetmin(dists);
            if(data.cls(index)!=data.cls(ii)) total++;
        }
...
#pragma omp parallel for shared(dists)
    ...
    float estimate_errors_sampled(IModel &classifier,floatarray &data,intarray &classes,int n=1000) {
        ...
        for(int i=0;i<n;i++) {
            floatarray v;
            int row;
            do {row = rand()%classes.length();} while(classes(row)==-1);				//Gary Probably broke
            rowget(v,data,row);
            int pred = classifier.classify(v);
...
#pragma omp parallel for
    ...
    void confusion_matrix(intarray &confusion,IModel &classifier,floatarray &data,intarray &classes) {
        int npred = classifier.nclasses();
        int nclasses = colib::getmax(classes) + 1;
        confusion.resize(npred,nclasses);
        confusion.fill(0);
#pragma omp parallel for
    ...
    struct KnnClassifier : IModel {
        ...
        void add(floatarray &v,int c) {
            CHECK(getmin(v)>-100 && colib::getmax(v)<100);
            ASSERT(valid(v));
            ASSERT(v.rank()==1);
            CHECK(c>=0);
...
#pragma omp parallel for
    ...
    struct bitvec {
        ...
        void set(T *v,int nbits,T threshold) {
            reserve_bits(nbits);
            uint64 word = 0xf0f0f0f0f0f0f0f0;
            int index = 0;
            int bitindex = 0;
        ...
        template <class T>
        void set(narray<T> &v) {
            set(&v(0),v.length(),(getmin(v)+getmax(v))/2);
        }
        template <class T>
        ...
        int dist(bitvec &other) {
            int total = 0;
            int n = colib::getmin(nwords,other.nwords);
    ...
    struct BitNN : IModel {
        ...
        void info(int depth,FILE *stream) {
            ...
            iprintf(stream,depth,"nfeat %d nprotos %d nclasses %d k %d\n",
                    nfeat,prototypes.length(),colib::getmax(classes)+1,k);
        }
        void save(FILE *stream) {
            psave(stream);
            narray_write(stream,classes);
...
#pragma omp parallel for
    ...
    inline float sigmoid(float x) {
        return 1.0 / (1.0+exp(-colib::getmin(colib::getmax(x,-20.0),20.0)));
    }
    ...
    public:
        ...
        void info(int depth,FILE *stream) {
            ...
            if(w1.length()>0 && w2.length()>0) {
                iprintf(stream,depth,"w1 [%g,%g] b1 [%g,%g]\n",colib::getmin(w1),colib::getmax(w1),colib::getmin(b1),colib::getmax(b1));
                iprintf(stream,depth,"w2 [%g,%g] b2 [%g,%g]\n",colib::getmin(w2),colib::getmax(w2),colib::getmin(b2),colib::getmax(b2));
            }
        }
        ...
        void initRandom(int ninput,int nhidden,int noutput) {
            ...
            w2.resize(noutput,nhidden);
            b2.resize(noutput);
            float range = 1.0/colib::getmax(ninput,nhidden);
        ...
        void train(IDataset &ds) {
            ...
            niters = colib::getmax(1000,colib::getmin(10000000,niters));
            double err = 0.0;
...
#pragma omp parallel for
                ...
                if(!mlp_noopt) {
                    for(int i=0;i<nn/2;i++) {
                        ...
                        int nn = colib::getmin(colib::getmax(hidden_min,int(rlognormal(n,hidden_varlog))),hidden_max);
...
#pragma omp parallel for
        ...
        void train(IDataset &ds) {
            ...
            for(int round=0;round<nrounds;round++) {
                ...
                int maxhidden = colib::getmax(net->pgetf("hidden_min")+5,colib::getmax(20,int(perp/30.0)));
                net->pset("hidden_max",maxhidden);
                net->pset("hidden_hi",colib::getmin(100,maxhidden));
                // net->set("crossvalidate",0);

                // Subsample 10x the perplexity; more doesn't really
                // make sense.
                int nsubsample = int(perp*10);
                nsubsample = colib::getmax(1000,colib::getmin(weights.length(),nsubsample));
                intarray subsample;
                ...
                int errs,errs1;
                if(1) {
                    int n = colib::getmin(1000,ds.nsamples());
                    errs=0;
                    errs1=0;
                    floatarray v;
                    for(int i=0;i<n;i++) {
        ...
        float outputs1(floatarray &result,floatarray &v) {
            ...
            for(int round=0;round<models.length();round++) {
                models(round)->outputs(p,v);
                result(arggetmax(p)) += alphas(round);
            }
            result /= sum(result);
            return 0.0;
        }
...
#pragma omp parallel for
    ...
    void init_glclass() {
        ...
        component_register<MappedClassifier>("mapped", true);
