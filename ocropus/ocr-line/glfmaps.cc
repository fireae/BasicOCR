...
namespace glinerec {
    struct SimpleFeatureMap : IFeatureMap {
        ...
        virtual void setLine(bytearray &image_) {
            ...
            dclear(0);
            dshow(line,"yyy");
            pad_by(line,pad,pad,getmax(line));
            ...
            if(strchr(ftypes,'D') || strchr(ftypes,'G') || strchr(ftypes,'M')) {
                ...
                if(strchr(ftypes,'G') || strchr(ftypes,'M')) {
                    ...
                    float n = getmax(1e-5,getmax(getmax(dt_x),getmax(dt_y)));
                    dt_x /= n;
                    dt_y /= n;
                    ssigmoid(dt_x);
                    ssigmoid(dt_y);
        ...
        void extractFeatures(floatarray &v,rectangle b,bytearray &mask) {
            ...
            if(strchr(ftypes,'b')) {
                ...
                push_array(v,u);
                CHECK(getmin(v)>=-1.1 && getmax(v)<=1.1);
            }
            if(strchr(ftypes,'h')) {
        ...
        void extractFeaturesAA(floatarray &v,rectangle b,bytearray &mask,
            ...
            float s = getmax(sub.dim(0),sub.dim(1))/float(csize);
            ...
            float maxval = getmax(fabs(getmax(v)),fabs(getmin(v)));
        ...
        void extractFeaturesNonAA(floatarray &v,rectangle b,bytearray &mask,
            ...
            int xm = mask.dim(0)/2;
            int ym = mask.dim(1)/2;
            int r = context*getmax(b.width(),b.height());
            v.resize(csize,csize);
            v = 0;
            for(int i=0;i<csize;i++) {
                for(int j=0;j<csize;j++) {
