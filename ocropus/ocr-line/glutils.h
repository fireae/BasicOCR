...
#pragma omp parallel for
    ...
    void knn_posterior(floatarray &posterior,intarray &classes,floatarray &costs,int k) {
        ...
        for(int i=0;i<costs.length();i++)
            nbest.add(i,-costs(i));
        posterior.resize(getmax(classes)+1);
        posterior.fill(0);
    ...
    void dprint(intarray &a,int n) {
        if(a.rank()==1) {
            ...
            for(int i=0;i<getmin(n,a.dim(0));i++) {
                printf("%4d [",i);
                int j;
                for(j=0;i<getmin(n,a.dim(j));i++) {
                    if(j>0) printf(" ");
                    printf("%d",a(i,j));
                }
                if(j<a.dim(1)) printf("...");
    ...
    void fit_into_box(bytearray &out,bytearray &in,rectangle b,int background=0) {
        ...
        float scale = getmax(xscale,yscale);
    ...
    void center(bytearray &out,bytearray &in,int size=32,int background=0) {
        ...
        point c = centroid(in);
        // in(c.x,c.y) = 96;
        int rx = getmax(c.x-b.x0,b.x1-c.x+1);
        int ry = getmax(c.y-b.y0,b.y1-c.y+1);
        int r = getmax(rx,ry);
        float scale = float(r)/(size/2);
        scale = getmax(scale,1.0);
