...
// Tess includes
#include "tesseract/cutil/tordvars.h"
#include "tesseract/ccmain/control.h"
#include "tesseract/ccmain/tessvars.h"
#include "tesseract/ccmain/tessbox.h"
#include "tesseract/ccmain/tessedit.h"
#include "tesseract/image/imgs.h"
#include "tesseract/textord/edgblob.h"
#include "tesseract/textord/makerow.h"
#include "tesseract/textord/wordseg.h"
#include "tesseract/ccmain/output.h"
#include "tesseract/ccmain/tstruct.h"
#include "tesseract/textord/tessout.h"
#include "tesseract/wordrec/tface.h"
#include "tesseract/classify/adaptmatch.h"
#include "tesseract/ccmain/baseapi.h"
#include "tesseract/cutil/globals.h"
...
namespace {
    ...
    void fill_lattice(IGenericFst &lattice, nustring &text) {
        floatarray costs;
        intarray ids;
        makelike((narray<float>)costs, (narray<nuchar>)text);
        makelike((narray<int>)ids, (narray<nuchar>)text);
...
#undef tess_segment_pass2
        ...
        void train(nustring &chars,intarray &orig_csegmentation) {
            ...
            int n = colib::getmax(csegmentation);
		...
		void tesseract_recognize_blockwise(
            ...
            for(int i = 1 /* RegionExtractor weirdness */; i < e.length(); i++) {
                ...
                pass_grayscale_image_to_tesseract(block_image);
                block_lists[i-1] = TessBaseAPI::FindLinesCreateBlockList();
                block_results[i-1] = TessBaseAPI::RecognitionPass1WithMonitor(block_lists[i-1], monitor1);
            }
            ...
            for(int i = 1 /* RegionExtractor weirdness */; i < e.length(); i++) {
                ...
                log_tesseract("pass 2 block", block_image);
                pass_grayscale_image_to_tesseract(block_image);
                block_results[i-1] = TessBaseAPI::RecognitionPass2WithMonitor(block_lists[i-1], block_results[i-1], monitor1);
                extract_result_from_PAGE_RES(text[i-1],
                                             bboxes[i-1],
                                             costs[i-1],
                                             *block_results[i-1]);
		...
		void tesseract_recognize_blockwise_and_split_to_lines(
            ...
            tesseract_recognize_blockwise(whole_zone_bboxes, zone_text,
                                          zone_bboxes, zone_costs, gray, pseg, monitor1);
	...
	void tesseract_recognize_blockwise_and_split_to_lines(
            ...
            intarray &pseg,
			const char *path) {
        autodel<TesseractWrapper> tess(new TesseractWrapper(path));
        tess->tesseract_recognize_blockwise_and_split_to_lines(text, bboxes, costs, gray, pseg);
    }
	...
	void tesseract_recognize_blockwise_and_split_to_lines(
			...
			ETEXT_DESC *monitor1) {
        autodel<TesseractWrapper> tess(new TesseractWrapper(path));
        tess->tesseract_recognize_blockwise_and_split_to_lines(text, bboxes, costs, gray, pseg, monitor1);
    }
...
namespace ocropus {
    void tesseract_recognize_blockwise(RecognizedPage &result, colib::bytearray &gray, colib::intarray &pageseg) {
		...
		void tesseract_recognize_blockwise(const char *path, 
        ...
        narray<narray<rectangle> > bboxes;
        narray<floatarray> costs;
        tesseract_recognize_blockwise_and_split_to_lines(text, bboxes, costs, gray, pageseg, path);
        result.setWidth(gray.dim(0));
        result.setHeight(gray.dim(1));
        int nonempty_count = 0;
        for(int i = 0; i < text.length(); i++) {
	...
	void tesseract_recognize_blockwise(const char *path, 
        ...
        narray<narray<rectangle> > bboxes;
        narray<floatarray> costs;
        tesseract_recognize_blockwise_and_split_to_lines(text, bboxes, costs, gray, pageseg, path, monitor1);
        result.setWidth(gray.dim(0));
        result.setHeight(gray.dim(1));
        int nonempty_count = 0;
        for(int i = 0; i < text.length(); i++) {
	...
	void tesseract_init_with_initdata_and_language(const char *path, const char *language) {
#ifdef GOOGLE_INTERNAL
        tess_instance = new TessBaseAPI();
        if(!getenv("TESSDATA_PREFIX"))
            throw "you must set TESSDATA_PREFIX when running in the Google environment";
        tess_instance->Init(0,language);
#else
        TessBaseAPI::InitWithLanguage(path,0,language,0,false,0,0);
#endif
    }
