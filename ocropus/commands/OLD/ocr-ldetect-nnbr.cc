...
struct Rectangle {
    ...
    void include(int x,int y) {
        if(x0>x1) {
            ...
            x0 = colib::getmin(x,x0);
            y0 = colib::getmin(y,y0);
            x1 = colib::getmax(x+1,x1);
            y1 = colib::getmax(y+1,y1);
        }
    }
    int width() {
        return x1-x0;
...
inline int iclamp(double x,int n) {
    return colib::getmax(0,colib::getmin(int(x+0.5),n-1));
}

void fit_into(floatarray &out,bytearray &in) {
    double scale = colib::getmax(in.dim(0)/double(out.dim(0)),
                       in.dim(1)/double(out.dim(1)));
    int dx = int(colib::getmax((in.dim(0)/scale-out.dim(0))/2,0));
    int dy = int(colib::getmax((in.dim(1)/scale-out.dim(1))/2,1));
    fill(out,0);
