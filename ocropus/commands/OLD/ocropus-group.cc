...
#define MAXFLOAT 3.40282e+038///3.40282347e+38F
...
struct Rect {
    ...
    void include(int x,int y) {
        if(x0>x1) {
            ...
            x0 = colib::getmin(x0,x);
            y0 = colib::getmin(y0,y);
            x1 = colib::getmax(x1,x+1);
            y1 = colib::getmax(y1,y+1);
        }
    }
...
int main(int argc,char **argv) {
    ...
    try {
        ...
        for(int i=1;i<nlabels;i+=offset) {
            printf("i=%d\n", i);
            for(int j=i;j<colib::getmin(nlabels,i+maxcomp);j++) {
                if (maxonly==1) {
                    j=colib::getmin(nlabels,i+maxcomp)-1;
                    offset=j-i+1;
                }
                Rect r;
                bool skip = false;
                ...
                if(baseline) {
                    ...
                    float py = bm*x+bb;
                    float pyd = bm*x+bd;
                    float error = colib::getmin(fabs(py-y),fabs(pyd-y));
