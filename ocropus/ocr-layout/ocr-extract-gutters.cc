...
void ColSeparators::findGutters(rectarray &columns,
    ...
    word_spacing = getmin(word_spacing, max_space);
    word_spacing = getmax(word_spacing, min_space);
    ...
    for(int i=0; i<wlen; i++){
        ...
        if( (getmax(left_neighbors, right_neighbors) >= maxthreshold) &&
            (getmin(left_neighbors, right_neighbors) >= minthreshold) ){
...
void ColSeparators::filterOverlaps(rectarray &colboxes, rectarray &colcandidates){
    ...
    for(int i=0; i<hlen; i++){
        rectangle temp = colheap.extractgetmax();
