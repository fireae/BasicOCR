...
namespace ocropus {
        ...
        for(int j = 0; j < imheight; j++){
            for(int i = 0; i < imwidth; i++){
                ...
                if((current_run_length_fg > 0) && (end_fg || (i == imwidth-1))){
                    current_run_length_fg = getmin(current_run_length_fg, MAX_LEN);
                ...
                if((current_run_length_bg > 0) && (end_bg || (i == imwidth-1))){
                    current_run_length_bg = getmin(current_run_length_bg, MAX_LEN);
    ...
    void ZoneFeatures::mainDiagRunLengths(floatarray &resulthist,
        ...
        for(int i = 0; i < imwidth + imheight; i++){
            for(int j = 0; j < getmin(imwidth, imheight); j++){
                if(i < imwidth){
                    if(j < i+1)
                        pix = image(i-j, j);
                    else
    ...
    void ZoneFeatures::concompHist(floatarray &result,
        ...
        for(int i=0, l=concomps.length(); i<l; i++){
            ...
            histogram_width(getmin(width, MAX_LEN)-1)++;
            histogram_height(getmin(height, MAX_LEN)-1)++;
            histogram_2D(getmin(width, MAX_LEN)-1,getmin(height, MAX_LEN)-1)++;
        }
