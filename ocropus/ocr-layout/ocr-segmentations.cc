...
namespace ocropus {
    ...
    void extract_lines(narray<bytearray> &lines,narray<rectangle> &rboxes,intarray &image) {
        ...
        for(int ki=0;ki<keys.length();ki++) {
            ...
            rboxes[ki].x0 = colib::getmax(0, box.x0 - padding);
            rboxes[ki].y0 = colib::getmax(0, box.y0 - padding);
            rboxes[ki].x1 = colib::getmin(image.dim(0), box.x1 + padding);
            rboxes[ki].y1 = colib::getmin(image.dim(1), box.y1 + padding);
    ...
    void rescale_if_needed(bytearray &bin_line,
        ...
        int threshold = (colib::getmin(gray_line) + colib::getmax(gray_line)) / 2;
    ...
    void concat_segmentation(colib::intarray &dst, colib::intarray &src,
                             int x, int y) {
        int d = colib::getmax(dst);
        int i_min = colib::getmax(0, -x);
        int j_min = colib::getmax(0, -y);
        int i_cap = colib::getmin(src.dim(0), dst.dim(0) - x);
        int j_cap = colib::getmin(src.dim(1), dst.dim(1) - y);
    ...
    void get_recoloring_map(intarray &recolor, intarray &image) {
        ...
        narray<bool> used(colib::getmax(image) + 1);
        fill(used, false);
        for(int i = 0; i < image.length1d(); i++)
