...
namespace ocropus {
    ...
    void WhitespaceCover::WState::update(WhitespaceCover *env) {
        ...
        for(int i=0;i<matches->length();i++) {
            ...
            if(nmatches->length()==1) {
                ...
                nbounds.x0 = colib::getmin(nbounds.x0,r.x1); //<?= r.x1;
                nbounds.y0 = colib::getmin(nbounds.y0,r.y1); //<?= r.y1;
                nbounds.x1 = colib::getmax(nbounds.x1,r.x0); //>?= r.x0;
                nbounds.y1 = colib::getmax(nbounds.y1,r.y0); //>?= r.y0;
            }
        }
        bounds = nbounds;
        matches = nmatches;
    ...
    void WhitespaceCover::compute() {
        ...
        for(int iter=0;;iter++) {
            ...
            CState state;
            state = queue.extractgetmax(); //Extract 'bounds' rect. with max. area
