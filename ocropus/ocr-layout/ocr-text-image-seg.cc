...
namespace ocropus {
    ...
    int TextImageSegByLogReg::getColor(floatarray &prob_map, int index){
        ...
        byte red = byte(255*(colib::getmax(prob_map(index,text),
                                 prob_map(index,table))));

        byte green = byte(255*(colib::getmax(prob_map(index,halftone),
                                   colib::getmax(prob_map(index,logo),
                                       prob_map(index,noise)))));

        byte blue = byte(255*(colib::getmax(prob_map(index,drawing),
                                  colib::getmax(prob_map(index,math),
