...
namespace ocropus {
    ...
    void PageFrameRAST::PFStateBasic::update(PageFrameRAST &pf,
        ...
        interval minq12 = colib::getmin(qleft,qright);
        if(minq12 < pf.min_q)
            quality = 0;
    ...
    void PageFrameRAST::cleanup(bytearray &out,bytearray &in){
        ...
        pageframe.x0 = colib::getmin(line_bb.x0,zone_bb.x0);
        pageframe.x1 = colib::getmax(line_bb.x1,zone_bb.x1);
        pageframe.y0 = colib::getmin(line_bb.y0,zone_bb.y0);
        pageframe.y1 = colib::getmax(line_bb.y1,zone_bb.y1);
